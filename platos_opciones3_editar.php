...
$id2   = $_GET['id2'];
$id3   = $_GET['id3'];
$qryAg = "SELECT *, (select nombre from _menus b where b.id=a.idmenu) as nplato, (select nombre from _menus_opciones c where c.id=a.idopcion) as nopcion, (select nombre from _menus_subopciones d where d.id=a.idopcion2) as nopcion2 FROM _menus_subopciones2 a WHERE id='$id' and idmenu='$idm' and idopcion='$id2'";
echo $qryAg;
		...
		<div class="container" style="margin-top: 175px;">
			<div class="row">
				<div class="col-md-7 box-admin-opt">
					<form action="platos_subopciones2_grabar.php" method="POST" accept-charset="utf-8">
					...
					<input type="hidden" name="idopcion" value="<?php echo $_GET["id2"];?>">
					<input type="hidden" name="idopcion2" value="<?php echo $_GET["id3"];?>">

					<div class="row">
						<div class="col-md-8">
							<h5 class="mt-0 mb-3 pl-2 text-info"><b><a href="platos_opciones2_editar.php?id=<?php echo $_GET["id3"];?>&idm=<?php echo $rowR["idmenu"];?>&id2=<?php echo $_GET["id2"];?>" class="text-info">< Editar Opción</a></b></h5>
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-md-12">
							<label>Plato</label><br>
							<h4><?php echo $rowR["nplato"];?>:&nbsp; <?php echo $rowR["nopcion"];?>:&nbsp; <?php echo $rowR["nopcion2"];?></h4>
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-md-12">
				...
				<div class="col-md-5">
					<div class="box-admin-opt">
						...
						<?php 
						$qryOp = "SELECT * FROM _menus_subopciones3 WHERE status!='E' and idmenu='$idm' and idopcion='$id2' and idopcion2='$id3' ORDER by status, nombre";
						...
						while ($rowOp = $rsOp->fetch_assoc()){
							...
							<div class="row py-2 <?php if ($van%2==0){ echo "bg-muted"; } ?>" style="<?php echo $bgf; ?>; border: 1px solid #eee; border-top: 0px;">
								<div class="col-7">
									<a href="#" data-toggle="modal" data-target="#boxEdit<?php echo $rowOp["id"];?>">
							  	...
							  	<div class="modal-dialog modal-dialog-centered position-relative" role="document">
							    	<div class="modal-content" style="background: linear-gradient(#f4f6f9, #dae4f4);">
										...
										<div class="modal-body p-4">
											<form action="platos_subopciones3_grabar.php" method="POST" accept-charset="utf-8">
											<input type="hidden" name="acc" value="edit">
											<input type="hidden" name="id"  value="<?php echo $rowOp["id"];?>">
											<input type="hidden" name="idopcion" value="<?php echo $_GET["id2"];?>">
											<input type="hidden" name="idopcion2" value="<?php echo $_GET["id3"];?>">
											<input type="hidden" name="idopcion3"  value="<?php echo $_GET["id"];?>">
